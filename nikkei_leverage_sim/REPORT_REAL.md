...
## 10. 同期間「ただ買って持つ」基準との比較 — 使える資本 ¥10M を分母にした公平比較

「本戦略の小細工（レバ＋条件付き積み増し＋利確）は、**淡々と買って持つだけ**に対して何を買っているのか」を問う。
**比較の分母は ¥100M（自己資金）ではなく、実際に使う資本 ¥10M（建玉上限）**にとる。本戦略は ¥100M 口座でも
市場に晒すのは ≤¥10M で、残り ¥90M は遊休担保にすぎないため、¥100M で割るとリターンもDDも実態より小さく見えるからだ。
（¥10M は PR#5 が特定した「レバ1倍＝追証が一度も出ない生存ライン」。同じ損益経路を ¥10M 自己資金で再現できる。）

### 10-1. ★公平比較：使える資本 ¥10M を分母にする

3者とも **¥10M を市場に投じる**前提に揃えた正面比較（本戦略は実 equity を ¥10M 資本へ取り直し＝遊休 ¥90M を除外）：

| 戦略（資本 ¥10M） | 期末価値 | 総リターン | CAGR | 最大DD% | 最大DD額 | 最悪時残高 |
|---|---:|---:|---:|---:|---:|---:|
| **本戦略**（積み増し＋利確） | ¥20.74M | +107.4% | 6.25% | **46.3%** | ¥6.32M | ¥7.33M |
| **1570.T 定額積立(DCA)** | ¥70.66M | **+606.6%** | 17.65% | 46.9% | ¥15.18M | ¥8.64M |
| **N225 定額積立(DCA)** | ¥28.11M | **+181.1%** | 8.97% | **24.6%** | ¥4.40M | ¥9.35M |

（本戦略の行は full 版の日次 equity を ¥10M 資本へ取り直して算出。fast 版も実現益 +¥9.99M=+99.9%・DD同程度でほぼ同じ。）

- **本戦略の「低ドローダウン」は分母のトリックだった。** ¥100M 割りなら 5% だが、**実際に使う ¥10M で割れば最大DDは ~46%**。
  最悪時には自己資金 ¥10M が ¥7.3M まで削られている（−27%）。「資本保全型・低DD」という売り文句は、遊休 ¥90M で薄めた見かけにすぎない。
- **同じ ¥10M をただ 1570.T に積み立てるだけ(DCA)と、ほぼ同じ ~46% のDDを負いながら、本戦略のリターンは 1/6**（+107% vs +606.6%）。
- **無レバの N225 を同額積み立てる方が、DDは半分（24.6%）・リターンは +181% で本戦略を全項目（リターン・DD・CVaR）で圧勝。**
  レバETF特有の逓減を避けられる分、リスク当たりでむしろ優位。

### 10-2. 参考：一括 buy & hold（¥10M を初日に全額）

| 戦略（資本 ¥10M） | 期末価値 | 総リターン | 最大DD% |
|---|---:|---:|---:|
| 1570.T 一括 B&H | ¥129.19M | **+1,191.9%** | 55.4% |
| N225 一括 B&H | ¥41.89M | +318.9% | 31.8% |

- 1570.T 終値 **5,625 → 72,670 ＝ ×12.92**（N225 は 15,909→66,641 ＝ ×4.19）。強い上昇トレンドで2倍レバの逓減を複利が上回った稀有な期間。
- 一括は +1,191.9% と最大だが最大DD 55%（コロナで半値近く）。定額積立はそれを +606.6%／DD 46.9% に和らげる＝DCA 教科書どおりの「上げを取り逃す代わりにDDが浅くなる」挙動。

### 10-3. まとめ — 本戦略の立ち位置

- **正しい分母（使える資本 ¥10M）で見ると、本戦略はリターンで定額積立に大敗（1570.T 比 1/6）し、ドローダウンでも無レバ N225 定額積立に負ける。** 前節までの「資本保全では勝っている」という評価は ¥100M 分母の錯覚で、**正しい分母では保全効果すら素の指数の定額積立に劣る**。
- 利確で勝者を早降りし、押し目買い（平均回帰）で大トレンドに逆らう設計そのものが、淡々と積み立てて持つだけに劣る。本戦略に独自の価値が出るのは、**レバETF を使わざるを得ず・かつ ¥10M 枠での実現益の刻みに意味がある**ごく限定的な状況に絞られる。
- **重要**: この期間は2倍レバETFに歴史的な追い風だった。**逆風（長期下落・レンジ）局面では B&H・定額積立の優位は容易に逆転する**（とくにレバETFは逓減が効く）。評価は局面依存。

> 注: 数値は追跡済みの実価格系列・equity（`outputs_real/daily.csv`、3,032営業日）から、エンジンと同一のメトリクス関数で算出。一括 B&H の総リターン・DD% は `benchmarks.json`（¥100M基準）と一致（リターン率・DD% は資本額に依らずスケール不変）。本戦略の ¥10M 資本ベースは equity から遊休 ¥90M を除いて再計算（最低残高 ¥7.33M>0 で口座は破綻しない）。自動生成 `report.md §4` は ¥100M 分母のままだが、同じ理由の警告と本節への参照を明記している。
