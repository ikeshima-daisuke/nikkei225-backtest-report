...
同一期間の対象を **2 通り**で並べる。いずれも本戦略と同じ総資金の投下を仮定する。

- **一括 buy & hold**: 初日に全額を投じて放置（最も積極的な資金投入）。
- **定額積立 (DCA)**: 同じ総額を**全営業日に均等額で買い続けて持つ**だけ（レバなし・利確なし）。本戦略と同じく
  「一度に張らず積み増す」ので、執行ペースの面では本戦略により近い等質な参照点。

### 10-1. ¥10M を投下した場合の最終損益

| 戦略 | 投下資金 | 期末価値 | 損益 | リターン |
|---|---:|---:|---:|---:|
| **本戦略**（最大建玉 ¥10M・平均 ¥3M） | ¥100M（equity） | ¥109.99M | **+¥9.99M** | +10.0% |
| 1570.T 一括 buy & hold | ¥10M | ¥129.19M | **+¥119.19M** | **+1,191.9%** |
| **1570.T 定額積立 (DCA)** | ¥10M | ¥70.66M | **+¥60.66M** | **+606.6%** |
| N225 一括 buy & hold | ¥10M | ¥41.89M | +¥31.89M | +318.9% |
| N225 定額積立 (DCA) | ¥10M | ¥28.11M | +¥18.11M | +181.1% |

- 1570.T 終値: **5,625 → 72,670 ＝ ×12.92**。2014 年初に ¥10M 分を一括で買って放置すれば ¥129.2M（+¥119.2M）。
- **定額積立でも +606.6%（¥10M→¥70.66M）と本戦略(+10%)を桁違いに上回る**。一括の半分強にとどまるのは、上昇相場では資金の大半が後半まで現金で待機し「上げの初期を取り逃す」ため（DCA の構造的特徴）。それでも「ただ毎日買って持つだけ」が本戦略を圧倒する。
- 参考: N225 は 15,909 → 66,641 ＝ ×4.19。2倍レバETF が ×12.92 と、強い上昇トレンドのおかげで逓減（ボラ・ドラッグ）を上回って複利が効いた稀有な期間。

### 10-2. 同一¥100M を投下した場合のリスク込み比較（自動生成 §4 と同値）

`outputs_real_fast/report.md §4` の自動生成表（同一の自己資金 ¥100M ベース）から、ドローダウン等のリスク指標込みで：

| 戦略 | 期末資産 | 総リターン | CAGR | 最大DD% | Sharpe | Sortino | Ulcer | CVaR95 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| **本戦略**（fast） | ¥109.99M | +10.0% | 0.83% | **5.0%** | 0.447 | — | — | — |
| 1570.T 一括 B&H | ¥1,291.91M | +1,191.9% | 23.70% | 55.4% | 0.718 | 0.699 | 19.87 | 6.14% |
| **1570.T 定額積立** | ¥706.61M | +606.6% | 17.65% | 46.9% | 0.721 | 0.716 | 11.22 | 4.12% |
| N225 一括 B&H | ¥418.89M | +318.9% | 12.64% | 31.8% | 0.666 | 0.654 | 9.70 | 3.09% |
| **N225 定額積立** | ¥281.13M | +181.1% | 8.97% | **24.6%** | 0.690 | 0.693 | 5.02 | 2.00% |

- **定額積立は一括 B&H より最大DD・Ulcer・CVaR が一貫して小さい**（1570.T: 55%→47%、Ulcer 19.9→11.2／N225: 32%→25%）。「上げを取り逃す代わりにDDが浅くなる」という DCA の教科書どおりの挙動が実データでも確認できる。
- **しかし本戦略の最大DD ≈5% は、無レバの定額積立(25〜47%)よりさらに桁違いに浅い**。これは本戦略が ¥100M の自己資金に対し平均 ¥3M（3%）しか張らないため。**「買って持つ」勢に金額で大敗するのは、ほとんど現金で待機しているからであり、戦略の優劣というより資金投入量の差**である点が DCA との対比で一段はっきりする。

### 10-3. まとめ — 本戦略の立ち位置

- **リターンでは一括 B&H＞定額積立＞＞本戦略**。3 版間の本戦略の ±¥1〜2M 差（フル版 +¥10.74M 等）は、B&H の +¥119M・DCA の +¥60M の前では誤差。
- 本戦略の存在意義は「リターン最大化」ではなく、**低エクスポージャ（平均 3%）で追証を回避しつつ小さく実現益を積む資本保全型**。同じ「積み増す」発想でも、無レバ全力の定額積立とはリスク・リターンの座標がまったく異なる。
- **重要**: この期間は 2倍レバETF にとって歴史的な追い風だった。**逆風（長期下落・レンジ）局面では一括 B&H・定額積立の優位は容易に逆転する**（とくにレバETFは長期下落・レンジで逓減が効く）。本戦略の低リターンは「強気相場で機会を取り逃した」とも「暴落で守られた」とも読め、評価は局面依存。

> 注: 上記 DCA 数値は追跡済みの実価格系列（`outputs_real/daily.csv` の `target_close`／`benchmark_close`、3,032営業日）から、エンジンと同一のメトリクス関数で算出（`benchmark.dca_curve` と一致）。一括 B&H 値は `benchmarks.json` と1円単位で一致することを確認済み。各版の `report.md §4`・`benchmarks.json` にも定額積立2行が追加済み。
